...
(declare-function narrow-to-defun "lisp" (&optional INCLUDE-COMMENTS))
...
The numeric argument is interpreted the following: One prefix
\\[universal-argument] means 'narrow on one top level up', two
prefixes (\\[universal-argument]\\[universal-argument]) means
'two levels up', etc.

Real numeric arguments (like \\[universal-argument] 3) should not
be used; they are divided by four and rounded up to determine the
number of levels 'up'."
  (unless (mod arg 4)
...
whichever applies first. Optional prefix argument N is used to
widen the scope of the narrowed region."
  (interactive "p")
...
(defun narrow-or-widen-dwim-clone (&optional n)
  "Clone buffer and do `narrow-or-widen-dwim'.
Pass N to `narrow-or-widen-dwim'. Return the new buffer."
  (interactive "p")
  (declare (interactive-only))
  (when (buffer-narrowed-p)
    (user-error "You have to widen the buffer to clone it"))
...
Prefix argument N widens the scope of the entity."
  (interactive "p")
  (declare (interactive-only))
  (when (buffer-narrowed-p)
